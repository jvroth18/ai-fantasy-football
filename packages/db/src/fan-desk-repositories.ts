...
export class FanDeskRepository {
  ...
  saveProfile(input: FanDeskProfileV1): FanDeskProfileV1 {
    ...
    if (existing && existing.teamId !== profile.teamId)
      throw new Error('Fan desk profile belongs to a different team');
    this.db
  ...
  getProfile(teamId: string): FanDeskProfileV1 | null {
    const row = this.db
      .select()
      .from(fanDeskProfiles)
      .where(eq(fanDeskProfiles.teamId, teamId))
      .get();
  ...
  markEmailed(postId: string, emailedAt: string): void {
    ...
    this.db
      .update(fanPosts)
      .set({ status: 'emailed', postJson: JSON.stringify(updated), emailedAt })
  ...
  listPosts(teamId: string, limit = 20): FanPostV1[] {
    return this.db
      .select()
      .from(fanPosts)
      .where(eq(fanPosts.teamId, teamId))
      .orderBy(desc(fanPosts.createdAt))
      .limit(Math.min(limit, 100))
      .all()
      .map((row) => fanPostV1Schema.parse(JSON.parse(row.postJson)));
  }

  queueEmail(
    input: Omit<typeof fanEmailOutbox.$inferInsert, 'id'> & { id?: string },
  ): StoredFanEmail {
    const id = input.id ?? randomUUID();
    this.db
      .insert(fanEmailOutbox)
      .values({ ...input, id })
      .run();
  ...
  updateEmail(
    ...
    this.db
      .update(fanEmailOutbox)
      .set({ status, errorMessage, sentAt })
      .where(eq(fanEmailOutbox.id, id))
      .run();
  }
  ...
  listEmails(teamId: string, limit = 20): StoredFanEmail[] {
    return this.db
      .select()
      .from(fanEmailOutbox)
      .where(eq(fanEmailOutbox.teamId, teamId))
      .orderBy(desc(fanEmailOutbox.createdAt))
      .limit(Math.min(limit, 100))
      .all();
  }
}
